...
import { portfolio, guardrails } from '../store/PaperStore.js';
export class RiskService {
    validate(plan, profile) {
        ...
        if (planned > posCap)
            reasons.push(`position ${planned.toFixed(2)} exceeds cap ${posCap.toFixed(2)}`);
        const estLoss = planned * (plan.exitRules.stopLossPct ?? 0.03);
        const maxDaily = pf.equity * profile.maxDailyLossPct;
        if (estLoss > maxDaily)
            reasons.push(`est loss ${estLoss.toFixed(2)} exceeds daily ${maxDaily.toFixed(2)}`);
        if (profile.requireKillSwitch && !gr.killSwitch)
            reasons.push('kill switch not armed');
